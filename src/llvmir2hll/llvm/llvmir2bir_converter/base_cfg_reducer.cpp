...
bool BaseCFGReducer::reduceCFG(ShPtr<CFGNode> cfg) {
  ...
  return sc->BFSTraverse(
      cfg, [this](const auto &node) { return this->inspectCFGNode(node); });
}

/**
 * @brief 遍历给定的控制流图节点 @a cfg
 * 并将化简所有节点直至控制流图中只包含一个节点。 当节点 @a cfg 在循环中时,
 * 仅化简该循环, 而不化简循环外的节点。
 *
 * @par 前置条件
 *  - @a cfg 不为 @c nullptr .
...
/**
 * @brief 检查给定的控制流图节点 @a node 是否可以被化简为结构化语句,
 * 如果可以，则将其与相应后继节点 化简为结构化语句.
 *
 * @returns 如果有节点被化简则返回 @c true 否则返回 @c false.
 *
 * @par 前置条件
 ...
 * @param succs @a node 化简得到节点的所有后继节点,
 * 不在该列表中的后继节点将被移除
 * @param append 如果设置为 @c true , 则将 @a body 中的语句添加到 @a node
 * 原有语句之后, 否则直接替换所有语句
 *
 * @par 前置条件
 *  - @a node 不为 @c nullptr .
 ...
 */
void BaseCFGReducer::reduceNode(ShPtr<CFGNode> node, ShPtr<Statement> body,
                                CFGNodeVector &succs, bool append) {
  PRECONDITION_NON_NULL(node);
 ...
 */
ShPtr<GotoStmt> BaseCFGReducer::getGotoStmt(const ShPtr<CFGNode> &node,
                                            const ShPtr<CFGNode> &target) {
  PRECONDITION_NON_NULL(node);
  PRECONDITION_NON_NULL(target);

  auto gotoStmt =
      GotoStmt::create(target->getBody(), node->getBody()->getAddress());
 ...
 *  - @a node 和 @a target 都不为 @c nullptr .
 */
ShPtr<BreakStmt> BaseCFGReducer::getBreakStmt(const ShPtr<CFGNode> &node,
                                              const ShPtr<CFGNode> &target) {
  PRECONDITION_NON_NULL(node);
  PRECONDITION_NON_NULL(target);
...
/**
 * @brief 生成一条新的 @c continue 语句, 该语句从节点 @a node 跳转到节点 @a
 * target .
 *
 * @par 前置条件
 *  - @a node 和 @a target 都不为 @c nullptr .
 */
ShPtr<ContinueStmt>
BaseCFGReducer::getContinueStmt(const ShPtr<CFGNode> &node,
                                const ShPtr<CFGNode> &target) {
  PRECONDITION_NON_NULL(node);
  PRECONDITION_NON_NULL(target);
...
/**
 * @brief 生成一条新的 @c if 条件语句, 条件语句的条件为 @a cond, @c if
 * 分支代码块由 @a trueBody 给出,
 *        @c else 分支代码块由 @a falseBody 给出.
 *
 * 如果给定的 @a falseBody 为 @c nullptr 或只包含空语句,
 * 则生成的条件语句不会包含 @c else 分支.
 *
 * @par 前置条件
...
ShPtr<IfStmt> BaseCFGReducer::getIfStmt(const ShPtr<Expression> &cond,
                                        ...
                                        const ShPtr<Statement> &falseBody) {
...
/**
 * @brief 生成一条新的 @c switch 分支语句, 该语句的条件为 @a cond, 分支代码块由
 * @a clauses 给出. 其中 @a clauses 为 @c switch 语句的所有分支,
 * 每个分支由一组条件和这些条件下执行的 @c case 代码块组成.
 *
 * @par 前置条件
 *  - @a cond 和 @a clauses 均不为 @c nullptr .
 */
ShPtr<SwitchStmt>
BaseCFGReducer::getSwitchStmt(const ShPtr<CFGNode> switchNode,
                              const ShPtr<Expression> &cond,
                              const SwitchClauseBodyVector &clauses) {
  PRECONDITION_NON_NULL(switchNode);
  PRECONDITION_NON_NULL(cond);

  auto switchStmt =
      SwitchStmt::create(cond, nullptr, switchNode->getBody()->getAddress());

  for (const auto &clause : clauses) {
    auto conds = clause->first;
    auto clauseBody = clause->second;
    for (std::size_t i = 0, e = conds.size() - 1; i < e; ++i) {
      switchStmt->addClause(
          conds[i],
          EmptyStmt::create(
              nullptr, Statement::getLastStatement(clauseBody)->getAddress()));
    }
 ...
 */
ShPtr<WhileLoopStmt>
BaseCFGReducer::getWhileLoopStmt(const ShPtr<CFGNode> loopNode,
                                 const ShPtr<Expression> &cond,
                                 const ShPtr<Statement> &body) {
  PRECONDITION_NON_NULL(cond);
  PRECONDITION_NON_NULL(body);

  return WhileLoopStmt::create(cond, body, nullptr,
                               loopNode->getBody()->getAddress());
}
 ...
 */
BaseCFGReducer::SwitchClauseVector
BaseCFGReducer::getSwitchClauses(const ShPtr<CFGNode> &switchNode) {
  PRECONDITION_NON_NULL(switchNode);
  ...
  for (auto &caseIt : switchInst->cases()) {
    auto cond =
        sc->converter->convertConstantToExpression(caseIt.getCaseValue());
    ...
    if (existingClauseIt != mapNodeToClause.end()) {
      ...
      auto newClause = std::make_shared<SwitchClause>(ExprVector{cond}, succ);
  ...
  if (hasDefault) {
    auto defaultClause = std::make_shared<SwitchClause>(ExprVector{nullptr},
                                                        switchNode->getSucc(0));
 ...
 * @param[in] clause 分支目标节点, 若表示跳转到 @c if 语句的后继节点, 则可以为
 * @c nullptr .
 ...
 */
ShPtr<Statement>
BaseCFGReducer::getIfClauseBody(const ShPtr<CFGNode> &node,
 ...
 */
ShPtr<Statement> BaseCFGReducer::getGotoBody(const ShPtr<CFGNode> &node,
                                             const ShPtr<CFGNode> &target) {
  PRECONDITION_NON_NULL(node);
  PRECONDITION_NON_NULL(target);
 ...
 * @brief 生成一个新的布尔常量表达式.
 */
ShPtr<ConstBool> BaseCFGReducer::getBoolConst(bool value) const {
  return ConstBool::create(value);
}

/**
 * @brief 获取给定的表达式 @a expr 的逻辑非表达式.
 */
ShPtr<NotOpExpr> BaseCFGReducer::getLogicalNot(const ShPtr<Expression> &expr) const {
  return NotOpExpr::create(expr);
}

/**
 * @brief 获取给定的表达式 @a lhs 和 @a rhs 的逻辑与表达式.
 */
ShPtr<AndOpExpr> BaseCFGReducer::getLogicalAnd(const ShPtr<Expression> &lhs,
                              const ShPtr<Expression> &rhs) const {
  return AndOpExpr::create(lhs, rhs);
}

/**
 * @brief 获取给定的表达式 @a lhs 和 @a rhs 的逻辑或表达式.
 */
ShPtr<Expression> BaseCFGReducer::getLogicalOr(const ShPtr<Expression> &lhs,
                             const ShPtr<Expression> &rhs) const {
  return OrOpExpr::create(lhs, rhs);
}
 ...
 *  - @a node 不为 @c nullptr .
 */
ShPtr<Expression> BaseCFGReducer::getNodeCondExpr(const ShPtr<CFGNode> &node) const {
  return getValueExpr(node->getCond());
}

/**
 * @brief 获取给定的值 @a value 的表达式.
 */
ShPtr<Expression> BaseCFGReducer::getValueExpr(llvm::Value *value) const {
  return sc->converter->convertValueToExpression(value);
}
 ...
 *  - @a node 不为 @c nullptr .
 */
bool BaseCFGReducer::isLoopHeaderUnderAnalysis(
    const ShPtr<CFGNode> &node) const {
  PRECONDITION_NON_NULL(node);
  return hasItem(loopHeadersOnStack, node) && !loopHeaderStack.empty() &&
